...
- the static subset now covers `fmt.Print`/`fmt.Println`, multi-placeholder `fmt.Printf` for `%s`/`%d`, integer/string/boolean constants, scalar package variables, local scalar `var` declarations, integer expressions, string `len(...)`, string equality/inequality, logical conditions, recursion, main-package integer or string helper functions, simple loops with `break`/`continue`, conditionals, simple integer/string/expressionless `switch` statements, and local imported packages that expose integer or string helper functions plus package-level scalar state
