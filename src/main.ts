...
type TinyGoBuildRequestOverrides = Partial<Pick<TinyGoBuildRequest, 'optimize' | 'panic' | 'scheduler' | 'target'>>

type TinyGoBuildPlanInputs = {
  buildRequestOverrides: TinyGoBuildRequestOverrides | null
  workspaceFiles: Record<string, string> | null
}

type TinyGoTestHookAction = 'booting' | 'planning' | 'executing'
...
let activeTestHookAction: TinyGoTestHookAction | null = null
let buildStateVersion = 0

const textEncoder = new TextEncoder()
const textDecoder = new TextDecoder()

const buildDirectoryContentsFromTextEntries = (entries: Record<string, string>) => {
...
const ensureTestHookActionIdle = () => {
  if (activeTestHookAction !== null) {
    throw new Error(`wasm-tinygo test hook action already running: ${activeTestHookAction}`)
  }
}

const runWithTestHookAction = async <T>(action: TinyGoTestHookAction, run: () => Promise<T>) => {
  ensureTestHookActionIdle()
  activeTestHookAction = action
  try {
    return await run()
  } finally {
    activeTestHookAction = null
  }
}
...
const planBuild = async (
  ...
  try {
    ...
    const workspace = new PreopenDirectory('/workspace', buildDirectoryContentsFromTextEntries(workspaceFiles))
...
;(window as Window & { __wasmTinygoTestHooks?: TinyGoTestHooks }).__wasmTinygoTestHooks = {
  boot: async () => {
    ...
    await runWithTestHookAction('booting', async () => {
      await ensureEmception()
    })
  },
  plan: async () =>
    runWithTestHookAction('planning', async () => {
      try {
        setControlsLocked(true)
  ...
  execute: async () => {
    await runWithTestHookAction('executing', async () => {
      await executeBuildPlan()
    })
  },
