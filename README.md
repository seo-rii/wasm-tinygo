...
- The pure-browser static execution path now covers the starter compatibility subset used by the smoke tests: `fmt.Print`/`fmt.Println`, multi-placeholder `fmt.Printf` for `%s`/`%d`, integer and string constants, integer expressions, recursive helper functions, simple loops and conditionals, and local imported packages that expose integer or string helper functions.
