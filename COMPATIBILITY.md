...
This document describes the compatibility level of `wasm-tinygo` as of 2026-03-20.
...
| Build request/result contract | Working | Driver reads `tinygo-request.json`, writes `tinygo-result.json`, and the `tinygo` planner emits a minimal bootstrap manifest with normalized `compileInputs` and `bootstrapDispatch`, plus a separate `tinygo-frontend-input.json` handoff file whose compile groups now live under nested `sourceSelection` and whose compile toolchain now lives under nested `toolchain`; it embeds a normalized compile-input/dispatch JSON payload into the bootstrap translation unit shape, emits a target-specific minimal TinyGo root tree that the browser host materializes strictly from `bootstrapDispatch.materializedFiles`, omits duplicate `compileInputs.allFiles`, leaves dispatch/materialization sizes to be derived from the embedded manifest instead of mirrored through wasm exports or planner JSON, and the browser-side front-end now consumes that handoff to regenerate `tinygo-bootstrap.c` while deriving its final materialization set from nested `sourceSelection`, deriving target asset and runtime-support files from `materializedFiles` plus shared TinyGo root/target rules, reconstructing program/stdlib/imported groups from `entryFile` and `allCompile`, restoring default bootstrap translation-unit/object paths plus default target-profile `llvmTarget` / `linker` / `cflags` / `ldflags` when they are omitted from either the handoff or the emitted compile-unit/embedded manifest, and deriving the executable tool plan directly from `tinygo-compile-unit.json` without carrying dead bootstrap-only `imports`, `buildTags`, `materializedFiles`, `targetAssets`, `runtimeSupport`, `program`, or `imported` metadata in the front-end handoff |
...
8. run the separate backend WASI stage on `/working/tinygo-backend-input.json` to produce `/working/tinygo-backend-result.json`
9. materialize `/working/tinygo-lowered-sources.json`, `/working/tinygo-lowered-ir.json`, `/working/tinygo-lowered-bitcode.json`, `/working/tinygo-lowered/*.c`, `/working/tinygo-lowered-command-batch.json`, `/working/tinygo-lowered-artifact.json`, `/working/tinygo-command-artifact.json`, and `/working/tinygo-command-batch.json` from that backend result and execute the lowered and final command batches
10. read the embedded manifest payload back out of that wasm artifact in the browser host
...
- confirming that the terminal log includes `bootstrap roundtrip verified`
...
- browser-side TinyGo front-end handoff consumption that writes `tinygo-compile-unit.json` as the sole execution handoff in the built WASI driver

The relevant tests live in:

- [driver_test.go](/home/seorii/dev/hancomac/wasm-tinygo/internal/driver/driver_test.go)

### Browser smoke verification

The browser flow has been exercised end-to-end with headless Chromium:

1. boot `emception`
2. run the Go/WASI driver
3. run the browser-side TinyGo front-end handoff consumer
4. produce a 2-step `tinygo-bootstrap` build plan
5. execute the plan
6. generate `/working/out.wasm`

This is runtime verification, not yet a checked-in browser test suite.
